...
{	
	...
	// receive mode, irq enable
	nrf_w_reg(CONFIG,0x0F);
...
void nrf_print_registers(void)
{
	...
	uart_send_string("tx_addr_pipe0, lsb first = ");
	nrf_print_addr(addr);
	uart_send_byte('\n');
	
	nrf_r_addr(RX_ADDR_P0,addr);
	uart_send_string("tx_receive_pipe0_addr, lsb first = ");
	nrf_print_addr(addr);
	uart_send_byte('\n');
	
	nrf_r_addr(RX_ADDR_P1,addr);
	uart_send_string("rx_pipe1_addr, lsb first = ");
	nrf_print_addr(addr);
	uart_send_byte('\n');
